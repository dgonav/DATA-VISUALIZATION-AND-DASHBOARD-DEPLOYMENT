...
# --- Chart 5: Applications vs Enrolled over Time (grouped bar chart) ---
# This chart helps us understand how many applicants actually end up enrolling.
# A grouped bar chart lets us compare both values for the same year side by side.
st.subheader("Applications vs Enrolled Students Over Time")
apps_df = (
    filtered_df.groupby("Year")[["Applications", "Enrolled"]]
    .mean()
    .reset_index()
)

fig5, ax5 = plt.subplots(figsize=(12, 4))
x = range(len(apps_df["Year"]))
width = 0.35  # Bar width chosen so the two bars fit next to each other without overlap

ax5.bar(
    [i - width / 2 for i in x],
    apps_df["Applications"],
    width,
    label="Applications",
    color="#4C72B0"
)
...
ax5.set_xticklabels(apps_df["Year"])
ax5.set_xlabel("Year")
